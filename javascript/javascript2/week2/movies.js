...
movies = [
  ...
  const countNumberOfMovies = movies.filter(movie => movie.year >= 1980 && movie.year <= 1989).length
  console.log(countNumberOfMovies);
  
  //Create a new array that has an extra key called tag. T
  //The tag is based on the rating: Good (>= 7), Average (>= 4 and < 7), Bad (< 4)
  const createTagInMovies = movies.filter(movie => {
    if(movie.rating >= 7) {
  ...
  const getMovieRating = movies.filter(movie => movie.rating > 6)
  ...
  const totalMoviesWithKeywords = movies.filter(movie => movie.title.match(/alien/gi) || movie.title.match(/surfer/gi) || movie.title.match(/benjamin/gi)).length;
  ...
  function checkDuplicates(title) {
        ...
        wordsInTitle.some((word, index) => {
          const result = (wordsInTitle.indexOf(word) !== index);
            if(result){
            duplicatesInTitle.push(title);
            }
            return result;
          }); 
     }
    
  if(duplicatesInTitle.length > 0) {
      console.log(`Movies with duplicate words:`);
      duplicatesInTitle.forEach(title => console.log(title));
   } 
  
  //Calculate the average rating of all the movies using reduce. Optional
  
  const movieRating  = movies.map(movie => movie.rating);
  var averageRating = movieRating.reduce((total, rating) => total + rating) / movieRating.length;
  ...
  const movieTags = createTagInMovies.map(movie => movie.tag);
  const reducer = (count, tag) => {
    if (count[tag] == null) {
      count[tag] = 1;
    } else {
      ++count[tag];
  ...
  const countMovieTags = movieTags.reduce(reducer, {});
  console.log(countMovieTags);
